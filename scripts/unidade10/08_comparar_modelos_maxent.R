...
# Script 08: Comparar GLM, GAM, RF, BRT e MaxEnt
# ============================================================
...
r_max <- terra::rast(arquivos["MaxEnt"])
...
if (!terra::compareGeom(ref, r_max, stopOnError = FALSE)) {
  r_max <- terra::resample(r_max, ref, method = "bilinear")
}
...
r_stack <- c(r_glm, r_gam, r_rf, r_brt, r_max)

# ------------------------------------------------------------
# Consenso e incerteza
# ------------------------------------------------------------
...
terra::writeRaster(
  r_media,
  "resultados/unidade10/consenso_media_glm_gam_rf_brt_maxent.tif",
  overwrite = TRUE
)

terra::writeRaster(
  r_sd,
  "resultados/unidade10/incerteza_sd_glm_gam_rf_brt_maxent.tif",
  overwrite = TRUE
)
...
write_csv(
  df_pixel,
  "dados/unidade10/processados/comparacao_pixel_glm_gam_rf_brt_maxent.csv"
)
...
cor_modelos <- stats::cor(
  df_pixel[, c("GLM", "GAM", "RF", "BRT", "MaxEnt")],
  use = "complete.obs",
  method = "pearson"
)
...
write_csv(
  cor_modelos_tab,
  "tabelas/unidade10/correlacao_espacial_glm_gam_rf_brt_maxent.csv"
)
...
resumo_modelos <- df_pixel |>
  pivot_longer(
    cols = c(GLM, GAM, RF, BRT, MaxEnt, consenso_media, incerteza_sd),
    names_to = "produto",
    values_to = "valor"
  ) |>
  group_by(produto) |>
...
write_csv(
  resumo_modelos,
  "tabelas/unidade10/resumo_espacial_glm_gam_rf_brt_maxent.csv"
)

# ------------------------------------------------------------
# Bioma, área M e ocorrências
# ------------------------------------------------------------
...
bioma_raw <- sf::st_make_valid(bioma_raw)

bioma_union <- sf::st_union(bioma_raw)

bioma <- sf::st_as_sf(
  sf::st_sfc(
    bioma_union,
    crs = sf::st_crs(bioma_raw)
  )
)

bioma <- sf::st_transform(
  bioma,
  terra::crs(ref)
)
...
# ------------------------------------------------------------
# Mapas individuais com patchwork
# ------------------------------------------------------------

p_glm <- mapa_modelo(raster_para_df(r_glm, "GLM"), "GLM")
p_gam <- mapa_modelo(raster_para_df(r_gam, "GAM"), "GAM")
p_rf  <- mapa_modelo(raster_para_df(r_rf, "RF"), "Random Forest")
p_brt <- mapa_modelo(raster_para_df(r_brt, "BRT"), "BRT")
p_max <- mapa_modelo(raster_para_df(r_max, "MaxEnt"), "MaxEnt")

fig_modelos <- (p_glm + p_gam + p_rf) / (p_brt + p_max + patchwork::plot_spacer()) +
  patchwork::plot_layout(guides = "collect") +
  patchwork::plot_annotation(
    title = expression("Comparação de modelos para " * italic("Dinizia excelsa")),
    subtitle = "GLM, GAM, Random Forest, BRT e MaxEnt calibrados com as mesmas variáveis selecionadas por VIF"
  ) &
  theme(
...
ggsave(
  "figuras/unidade10/unidade10_comparacao_modelos_maxent.png",
  plot = fig_modelos,
  width = 15,
  height = 10,
  dpi = 600
...
g_consenso <- mapa_modelo(
  raster_para_df(r_media, "consenso_media"),
  expression("Consenso médio de adequabilidade para " * italic("Dinizia excelsa")),
  legenda = "Consenso"
)

ggsave(
  "figuras/unidade10/unidade10_consenso_media_glm_gam_rf_brt_maxent.png",
  plot = g_consenso,
  width = 9,
  height = 7,
  dpi = 600
...
df_incerteza <- raster_para_df(r_sd, "incerteza_sd")
...
g_incerteza <- ggplot() +
  ...
  labs(
    ...
    subtitle = "Desvio-padrão das predições GLM, GAM, Random Forest, BRT e MaxEnt",
    x = "Longitude",
    y = "Latitude"
  ) +
...
ggsave(
  "figuras/unidade10/unidade10_incerteza_sd_glm_gam_rf_brt_maxent.png",
  plot = g_incerteza,
  width = 9,
  height = 7,
  dpi = 600
...
ggsave(
  "figuras/unidade10/unidade10_sintese_consenso_incerteza_glm_gam_rf_brt_maxent.png",
  plot = fig_sintese,
  width = 14,
  height = 7,
  dpi = 600
...
message("Comparação GLM-GAM-RF-BRT-MaxEnt concluída.")
message("Raster de consenso salvo em: resultados/unidade10/consenso_media_glm_gam_rf_brt_maxent.tif")
message("Raster de incerteza salvo em: resultados/unidade10/incerteza_sd_glm_gam_rf_brt_maxent.tif")
message("Figura de comparação salva em: figuras/unidade10/unidade10_comparacao_modelos_maxent.png")
message("Figura de incerteza salva em: figuras/unidade10/unidade10_incerteza_sd_glm_gam_rf_brt_maxent.png")
