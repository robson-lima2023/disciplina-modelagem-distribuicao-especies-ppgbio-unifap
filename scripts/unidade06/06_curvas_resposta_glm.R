...
pacotes <- c(
  ...
  "purrr", "patchwork", "stringr", "tibble"
)
...
arquivo_dados <- "dados/unidade06/processados/dados_glm_dinizia_padronizados.csv"
...
if (!file.exists(arquivo_dados)) {
  stop("Dados padronizados não encontrados. Execute o Script 02 da Unidade 6.")
}
...
# ------------------------------------------------------------
# 7. Função auxiliar para nomes mais legíveis
# ------------------------------------------------------------
...
gerar_curva <- function(var_original, var_z) {
  ...
  tibble(
    ...
    adequabilidade = as.numeric(novo$adequabilidade)
  )
}
...
write_csv(
  curvas,
  "dados/unidade06/processados/curvas_resposta_glm.csv"
)
...
write_csv(
  resumo_curvas,
  "tabelas/unidade06/resumo_curvas_resposta_glm.csv"
)
...
g_curvas <- patchwork::wrap_plots(
  ...
  patchwork::plot_annotation(
    title = expression("Curvas de resposta parcial do GLM para " * italic("Dinizia excelsa")),
    subtitle = paste0(
...
ggsave(
  "figuras/unidade06/unidade06_curvas_resposta_glm.png",
  plot = g_curvas,
  width = 12,
  height = altura,
  dpi = 600
...
for (i in seq_along(lista_graficos)) {
  ...
  ggsave(
    filename = paste0(
      "figuras/unidade06/unidade06_curva_resposta_glm_",
      nome_arquivo,
      ".png"
    ),
...
message("Curvas de resposta parcial do GLM geradas com sucesso.")
message("Número de variáveis com curvas geradas: ", n_graficos)
message("Figura salva em: figuras/unidade06/unidade06_curvas_resposta_glm.png")
